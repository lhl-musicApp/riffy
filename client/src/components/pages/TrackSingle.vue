...
<!--       <div class="col-lg-6">
      ...
      <div class="track col-lg-6">
        <p>Track ID: {{ track.id }}</p>
        <p>Track Name: {{ track.track_name }}</p>
        <p>Created By: {{ track.isCreator }}</p>
        <p style="white-space: pre">{{ track.original_artist }}</p>
        <p>Influence: {{ track.soundcloud_link }}</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created () {
    this.$http.get('tracks/' + this.$route.params.id).then(response => {
